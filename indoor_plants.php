...
// Fetch products for Indoor Plants (category_id = 5)
$product_query = $conn->query("SELECT * FROM products WHERE category_id=5");
?>
...
<title>Indoor Plants - Flower Shop</title>
<style>
...
while($product = $product_query->fetch_assoc()) {
    // Get product image (first one from images table)
    ...
    if ($product['quantity'] > 0) {
        ...
        echo '<input type="hidden" name="redirect" value="indoor_plants.php">';
