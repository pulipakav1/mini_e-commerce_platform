...
CREATE TABLE receipts (
    ...
    CONSTRAINT fk_receipt_order FOREIGN KEY (order_id) REFERENCES orders(order_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE payment (
    ...
    CONSTRAINT fk_payment_order FOREIGN KEY (order_id) REFERENCES orders(order_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE employees (
    ...
    salary DECIMAL(10,2),
    hire_date DATE,
    employee_userid VARCHAR(150) UNIQUE,
    email VARCHAR(255),
...
CREATE TABLE images (
    ...
    CONSTRAINT fk_images_product FOREIGN KEY (product_id) REFERENCES products(product_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE cart (
    ...
    product_id INT NOT NULL,
    quantity INT NOT NULL CHECK (quantity > 0),
    CONSTRAINT fk_cart_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_cart_product FOREIGN KEY (product_id) REFERENCES products(product_id) ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE TABLE flower_education (
